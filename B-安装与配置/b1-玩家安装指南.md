...
**[我不想只玩模组，我想制作模组](b2-开发者安装指南.md)**

___

**[我不想只玩模组，我想为tModLoader做贡献](b3-贡献者安装指南.md)**
...
请注意，使用家庭共享安装Terraria的用户需要按照下面的[Steam家庭共享安装](b1-玩家安装指南.md#steam-family-share-installation)说明进行操作。
...
请按照以下说明切换到Steam中的不同版本tModLoader。如果您是GOG用户或需要手动安装特定版本，请改用[手动安装](b1-玩家安装指南.md#manual-installation)说明。
...
Linux/Mac：从Steam启动时，Steam显示游戏正在启动，然后按钮立即返回"播放"：这很可能是由于未使`.sh`文件可执行造成的。请重新访问上面[手动安装说明](b1-玩家安装指南.md#manual-installation)的步骤6。
