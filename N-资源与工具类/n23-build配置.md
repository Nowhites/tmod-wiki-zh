...
| weakReferences | mod将构建并可以本地交互的mod列表，但不一定需要启用。本质上是可选的modReferences，但需要mod作者在编程时更加小心以避免绑定错误。请参阅[跨Mod内容专家指南](Expert-Cross-Mod-Content.md)了解如何正确使用此功能。 |
...
| includeSource | 如果为true，则使用提取mod选项生成的解包mod将包含.cs文件，前提是`hideResources`仍为false。如果您希望自由分享您的代码，请使用此选项。如果您真的希望获得mod的社区支持，请考虑使用[GitHub](Intermediate-Git-&-mod-management.md)允许其他人为您mod贡献代码。 | false |
...
| translationMod <a name="translationmod"></a> | 如果为true，此mod将被视为为另一个mod提供翻译的mod。`modReferences`中列出的mod是此mod翻译的mod。[贡献本地化wiki页面](Contributing-Localization.md#contact-mod-developer-or-new-translation-mod)更详细地解释了这一点。当一起加载时，翻译mod将自动使用被翻译mod的本地化条目进行更新，从而使翻译mod作者能够轻松地将翻译更新到最新版本。这还将导致在Mods菜单中显示特殊图标，并添加一个[自定义创意工坊标签](https://steamcommunity.com/workshop/browse/?appid=1281930&requiredtags[]=Translation)，让用户更容易找到翻译mod。 | false |
