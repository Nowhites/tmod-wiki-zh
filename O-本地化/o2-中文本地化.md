...
[Original Page (English) | 原页面（英文）](o1-本地化指南.md)<br />
[Русская версия страницы | 俄文](o3-俄文本地化.md)
...
此指南将涵盖模组作者需要知道的本地化专题教程。如果你想要翻译一个已有的模组，或者是翻译tModLoader本身，请参阅[贡献本地化](Contributing-Localization.md)。
...
## 在1.4.3生成本地化文件
首先，我们需要一个较“老”的tModLoader来导出本地化文件。用Steam将tModLoader的测试版调至 `1.4.3-legacy`。（[切换tModLoader版本的教程](Basic-tModLoader-Usage-FAQ.md#switch-to-stable-tmodloader-or-to-preview-tmodloader))
...
## 切换至1.4.4并生成模组
用Steam将tModLoader的测试版调至 `无`。（[切换tModLoader版本的教程](Basic-tModLoader-Usage-FAQ.md#switch-to-stable-tmodloader-or-to-preview-tmodloader))
...
## 格式化字符串
模组作者可以使用[字符串格式化](https://learn.microsoft.com/en-us/dotnet/api/system.string.format?view=net-7.0#insert-a-string)在翻译中给要填的文本留出空位。这是C#的功能。你可以用方法 `string.Format` 或 `Language.GetTextValue` 来格式化字符串。章节[占位符](Contributing-Localization.md#placeholders)有关于此特性的更多信息。
